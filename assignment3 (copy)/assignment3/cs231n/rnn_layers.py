...
def rnn_backward(dh, cache):
  ...
  dprev = np.zeros_like(dh[:, T - 1, :])
  for t in reversed(xrange(1, T)):
      dx[:, t, :], dprev, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev, cache[t])
      dWx += dWxt
      dWh += dWht
      db += dbt

  dx[:, 0, :], dh0, dWxt, dWht, dbt = rnn_step_backward(dh[:, 0, :] + dprev, cache[0])
  dWx += dWxt
  dWh += dWht
  db += dbt
  ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = (i, f, o, g, next_c, prev_c, next_h, a, x, prev_h, Wx, Wh, b)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  i, f, o, g, next_c, prev_c, next_h, a, x, prev_h, Wx, Wh, b = cache
